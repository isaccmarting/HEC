...
// 求解偏微分方程的函数，这个是MPI+OpenMP混合并行方法
void JacobiMPIandOpenMP(Matrix mat); 
// 计算结果与真实值之间的L2-norm
...
int main(int argc, char* argv[])
{
    ...
    if(myRank == HEAD){
        if(provided == MPI_THREAD_SINGLE) printf("MPI library supports \'MPI_THREAD_SINGLE\'. \n"); 
        else if(provided == MPI_THREAD_FUNNELED) printf("MPI library supports \'MPI_THREAD_FUNNELED\'. \n"); 
        else if(provided == MPI_THREAD_SERIALIZED) printf("MPI library supports \'MPI_THREAD_SERIALIZED\'. \n"); 
        else if(provided == MPI_THREAD_MULTIPLE) printf("MPI library supports \'MPI_THREAD_MULTIPLE\'. \n"); 
    }
    // 设置每个节点（进程）拥有的线程数
...
void JacobiMPIandOpenMP(Matrix mat)
{
    ...
    {
        // temp[0]作为迭代的起始状态，用OpenMP的for语法并行执行循环
        ...
        #pragma omp barrier
        // 开始迭代
        while(iter < numIters && max_delta > eps){
            // There MUST be "#pragma omp barrier" here，因为之后会改变迭代变量iter等，导致部分线程满足循环退出条件，而导致死锁
            #pragma omp barrier
            // 由线程mater通讯其它进程，更新进程的矩阵区域的边界值，从相应进程获取上一轮中它们矩阵区域最后一行和第一行的结果
            #pragma omp master
            {
                if(last >= 0)
            ...
            // 用OpenMP的for语法并行执行循环，此外reduction语法得到线程中最大的delta即max_delta
            #pragma omp for private(i, j, delta) reduction(max:max_delta)
            for(i = start; i < end; i++)
                for(j = 1; j < width-1; j++){
            ...
            // 由线程mater通讯其它进程，整合所有进程的max_delta，更新全局的max_delta
            #pragma omp master
            {
                k = 1 - k; 
                delta = max_delta; 
            ...
            // 等所有线程完成到这里，因为之后循环条件判断需要用到max_delta等
            #pragma omp barrier
        }
